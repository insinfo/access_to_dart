...
Where ="(((TbAtColetivo.DataCol) Between [DATA INICIAL] And [DATA FINAL]) AND ((TbPessoa"
    ".DataDesligPes) Is Null Or (TbPessoa.DataDesligPes)>[DATA FINAL]) AND ((TbAtCole"
    "tivo.OfSESCFV)=Yes)) OR (((TbAtColetivo.DataCol) Between [DATA INICIAL] And [DAT"
    "A FINAL]) AND ((TbPessoa.DataDesligPes) Is Null Or (TbPessoa.DataDesligPes)>[DAT"
    "A FINAL]) AND ((TbAtColetivo.AtividadeSCFV)=Yes))"
Begin InputTables
    Name ="TbPessoa"
    Name ="TbAtColetivo"
