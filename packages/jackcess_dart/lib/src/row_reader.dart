...
class RowReader {
  ...
  Future<Map<String, dynamic>> readRow(DataPageRow row) async {
    ...
    for (var col in columns) {
        ...
        bool isNull = (nullMask[col.columnNumber ~/ 8] & (1 << (col.columnNumber % 8))) == 0;
        if (isNull) continue;

        if (col.fixedOffset != 0 || !isVariableLengthType(col.type)) {
            // Read Fixed Length
            int dataStart = 2 + col.fixedOffset; // OFFSET_COLUMN_FIXED_DATA_ROW_OFFSET = 2
            int len = col.length;
            ...
            int varColumnOffsetPos = nullMaskOffset - 4 - (col.variableColumnNumber * 2);
  ...
  dynamic _parseFixed(Uint8List data, int type) {
      ...
      switch(type) {
          ...
          case 4: return bytes.getInt32(0, Endian.little);
          case 3: return bytes.getInt16(0, Endian.little);
          case 2: return bytes.getInt8(0);
          case 8: return bytes.getFloat64(0, Endian.little);
          case 0x12: return bytes.getInt32(0, Endian.little);
          default: return "FixedType($type)";
      }
  }
  ...
  Future<dynamic> _parseVar(Uint8List data, int type) async {
      ...
      return "VarType($type, len=${data.length})";
  }
  ...
  Future<Uint8List> _readSingleLvalPage(int pageNum, int rowNum, int length) async {
  ...
  Future<Uint8List> _readMultiPageLval(int pageNum, int rowNum, int length) async {
